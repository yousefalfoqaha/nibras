...
public class DocumentSearchTool implements ToolCallback {
  ...
  public ToolDefinition getToolDefinition() {
    return ToolDefinition.builder()
        ...
        .inputSchema(JsonSchemaGenerator.generateForType(DocumentSearchToolInput.class))
        .build();
  }
  ...
  public String call(String toolInput) {
    ...
    List<Document> retrievedDocuments =
        this.searchService.search(searchDecisionResult.getUserQuery());
  ...
  private String buildDescription() {
    String documentTypesDescriptions =
        documentTypeRegistry.getDocumentTypes().stream()
            .map(DocumentType::toFormattedString)
            .collect(Collectors.joining("\n\n"));

    String attributeDescriptions =
        documentTypeRegistry.getDocumentAttributes().stream()
    ...
    String template =
        ...
        - List attributes (as valid JSON array of strings) mentioned in the conversation:
            - Put values that are mentioned in the conversation in 'conversationAttributes' as [ATTRIBUTE_NAME, ATTRIBUTE_VALUE, ...].
            - Put inferred or uncertain values (not in conversation) in 'guessedAttributes' as [ATTRIBUTE_NAME, ATTRIBUTE_VALUE, ...].
        ...
        {
          "query": "INSERT DOCUMENT TYPE DESCRIPTION KEYWORDS RELEVANT TO USER QUERY",
          "documentType": "INSERT_DOCUMENT_TYPE",
          "documentTypeYear": INSERT_INTEGER_YEAR,
          "conversationAttributes": ["INSERT_REQUIRED_ATTRIBUTE_NAME_1", "INSERT_REQUIRED_ATTRIBUTE_VALUE_1", "INSERT_REQUIRED_ATTRIBUTE_NAME_2", "INSERT_REQUIRED_ATTRIBUTE_VALUE_2",],
          "guessedAttributes": []
        }
        """;

    return String.format(template, documentTypesDescriptions, attributeDescriptions);
  }
}
