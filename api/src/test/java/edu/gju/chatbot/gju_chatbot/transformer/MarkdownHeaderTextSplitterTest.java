...
public class MarkdownHeaderTextSplitterTest {
  ...
  public void testExtractMarkdownHierarchy() {
    ...
    assertEquals("Introduction", (String) first.getMetadata().get(DocumentMetadataKeys.FORMATTED_BREADCRUMBS));
    assertEquals("Introduction > Background",
        (String) second.getMetadata().get(DocumentMetadataKeys.FORMATTED_BREADCRUMBS));
    assertEquals("Introduction > Background > Details",
        (String) third.getMetadata().get(DocumentMetadataKeys.FORMATTED_BREADCRUMBS));
    assertEquals("Introduction > Scope", (String) fourth.getMetadata().get(DocumentMetadataKeys.FORMATTED_BREADCRUMBS));
    ...
    assertEquals(3, ((Number) fourth.getMetadata().get(DocumentMetadataKeys.CHUNK_INDEX)).intValue());

    // --- New: check the breadcrumb objects themselves ---
    // Note: Breadcrumb[] is an inner static class of MarkdownTextSplitter
    var firstBreadcrumbs = (MarkdownTextSplitter.Breadcrumb[]) first.getMetadata()
        .get(DocumentMetadataKeys.BREADCRUMBS);
    var secondBreadcrumbs = (MarkdownTextSplitter.Breadcrumb[]) second.getMetadata()
        .get(DocumentMetadataKeys.BREADCRUMBS);
    var thirdBreadcrumbs = (MarkdownTextSplitter.Breadcrumb[]) third.getMetadata()
        .get(DocumentMetadataKeys.BREADCRUMBS);
    var fourthBreadcrumbs = (MarkdownTextSplitter.Breadcrumb[]) fourth.getMetadata()
        .get(DocumentMetadataKeys.BREADCRUMBS);
